...
export async function getAllContent<T>(
  ...
  const globPattern = contentType === 'events' ? '/events/*.md' : 
                     contentType === 'projects' ? '/projects/*.md' : 
                     contentType === 'courses' ? '/courses/*.md' : '/blog/*.md';
  ...
  const filteredFiles = Object.entries(contentFiles).filter(
    ([path]) => path.includes(`/content${globPattern}`)
  );
